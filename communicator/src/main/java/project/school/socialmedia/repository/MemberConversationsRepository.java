...
public interface MemberConversationsRepository extends JpaRepository<MemberConversations, Long> {
  ...
  List<MemberConversations> findByMemberId(String memberId);

  Page<MemberConversations> findByConversationId(Long conversationId, Pageable pageable);

  List<MemberConversations> findByConversationId(Long conversationId);

  @Query("SELECT EXISTS (SELECT 1 FROM MemberConversations mc WHERE mc.conversation.id = ?1 AND mc.member.id = ?2)")
  boolean isMemberOfConversation(Long conversationId, String memberId);

  void deleteByMemberId(String id);

  void deleteByMemberIdAndConversationId(String memberId, Long conversationId);

  MemberConversations findByMemberIdAndConversationId(String memberId, Long conversationId);
